...
from app.models import QuestionRequest, FinalResponse, GeneratedQuestion
...
def ask_question(request: QuestionRequest):
    ...
    if not intent:
        raise HTTPException(status_code=400, detail="Desculpe, não entendi a pergunta ou ela foge do escopo.")
    try:
        analysis_func = getattr(analyzer, intent)
    except AttributeError:
        raise HTTPException(status_code=500, detail="Função não implementada para a intenção detectada.")
